<template>
  <div class="border-t border-slate-200 bg-white flex-shrink-0 relative transition-all" :class="collapsed ? 'p-2' : 'p-3'">
    <!-- Collapsed View -->
    <div v-if="collapsed" class="flex flex-col items-center gap-2">
      <!-- Click & Long Press Zone (Avatar only) -->
      <div
        ...
        :title="`${name} (${roleText}) - Tahan 1.5 detik untuk Pengaturan Reset`"
      >
        <div class="relative">
          <div
            ...
            :class="[
              ...
              isPressing ? 'scale-95 ring-2 ring-rose-400' : 'hover:scale-105'
            ]"
          >
            {{ userInitial }}
          </div>
    ...
    >
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-[10px] font-semibold text-slate-400 leading-none mt-0.5 truncate">{{ roleText }}</p>
        </div>
      </div>
